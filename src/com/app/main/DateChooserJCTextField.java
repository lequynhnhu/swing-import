...
public class DateChooserJCTextField extends JCTextField {
    ...
    public DateChooserJCTextField() {
        this(getNowDate());
    }

    public DateChooserJCTextField(String dateString) {
        this();
    ...
    public DateChooserJCTextField(SimpleDateFormat df, String dateString) {
        this();
    ...
    public DateChooserJCTextField(Date date) {
        this("", date);
        //记忆原始日期时间
        initOriginalText(date);
    }

    public DateChooserJCTextField(String preLabel, Date date) {
        if (preLabel != null) {
    ...
    private class DateChooser extends JPanel implements ActionListener, ChangeListener {
        ...
        // 控制条配色------------------//
        Color controlLineColor = Color.magenta; // 控制条底色
        ...
        JCSpinner daySpin;
        JCSpinner hourSpin;
        JCSpinner minuteSpin;
        JCSpinner secondSpin;
        JCButton[][] daysButton = new JCButton[6][7];
        ...
        private JPanel createYearAndMonthPanal() {
            ...
            yearSpin = new JCSpinner(new SpinnerNumberModel(currentYear, startYear, lastYear, 1));
            yearSpin.setPreferredSize(new Dimension(52, 20));
          ...
          //这里修改只是为了获取时间
            ...
            JCLabel yearLabel = new JCLabel("年");
            yearLabel.setForeground(controlTextColor);
            result.add(yearLabel);

            monthSpin = new JCSpinner(new SpinnerNumberModel(currentMonth, 1, 12, 1));
            ...
            JCLabel monthLabel = new JCLabel("月");
            monthLabel.setForeground(controlTextColor);
            result.add(monthLabel);

            //如果这里要能够选择,会要判断很多东西,比如每个月分别由多少日,以及闰年问题.所以,就干脆把Enable设为false
            daySpin = new JCSpinner(new SpinnerNumberModel(currentMonth, 1, 31, 1));
            daySpin.setPreferredSize(new Dimension(40, 20));
            daySpin.setName("Day");
            ...
            JCLabel dayLabel = new JCLabel("日");
            dayLabel.setForeground(controlTextColor);
            result.add(dayLabel);

            hourSpin = new JCSpinner(new SpinnerNumberModel(currentHour, 0, 23, 1));
            hourSpin.setPreferredSize(new Dimension(40, 20));
            hourSpin.setName("Hour");
            hourSpin.addChangeListener(this);
            result.add(hourSpin);

            JCLabel hourLabel = new JCLabel("时");
            hourLabel.setForeground(controlTextColor);
            result.add(hourLabel);

            minuteSpin = new JCSpinner(new SpinnerNumberModel(currentMinute, 0, 59, 1));
            minuteSpin.setPreferredSize(new Dimension(40, 20));
            minuteSpin.setName("Minute");
            minuteSpin.addChangeListener(this);
            result.add(minuteSpin);

            JCLabel minuteLabel = new JCLabel("分");
            hourLabel.setForeground(controlTextColor);
            result.add(minuteLabel);

            secondSpin = new JCSpinner(new SpinnerNumberModel(currentSecond, 0, 59, 1));
            ...
            JCLabel secondLabel = new JCLabel("秒");
            hourLabel.setForeground(controlTextColor);
        ...
        private JPanel createWeekAndDayPanal() {
            ...
            for (int i = 0; i < 6; i++) {
                for (int j = 0; j < 7; j++) {
                    JCButton numberButton = new JCButton();
        ...
        private JPanel createButtonBarPanel() {
            ...
            ok.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    MainFrame.getInstance().startRun();
                }
            });
            panel.add(ok);

            JCButton cancel = new JCButton("取消");
        ...
        private JCPopupMenu createDialog() {
        	JCPopupMenu result = new JCPopupMenu();
        ...
        void showDateChooser(Point position) {
            Frame owner = (Frame) SwingUtilities.getWindowAncestor(DateChooserJCTextField.this);
            if (dialog == null) {
                dialog = createDialog();
            }
